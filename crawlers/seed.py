...
def scrape_seed_cards(driver):
    products = []
    time.sleep(3)
    product_cards = driver.find_elements("xpath", "//div[contains(@class, 'seed-card')]")
    for index in range(len(product_cards)):
        try:
            ...
            card.click()
            time.sleep(3)
            detail = scrape_seed_detail(driver)
            products.append(detail)
            driver.back()
            time.sleep(3)
        except Exception as e:
            print(f"[se-ed] Error at card {index}: {e}")
            try:
                driver.back()
            except:
                pass
...
def scrape_seed_all_pages(driver):
    ...
    while True:
        ...
        products = scrape_seed_cards(driver)
        if not products:
            break
        ...
        try:
            ...
            print(f"[se-ed] No next page or error: {e}")
            break
    return all_products
